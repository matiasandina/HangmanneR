...
You can play HangmanneR online at <https://matias-andina.shinyapps.io/HangmanneR/>. To run it locally:
...
- Stuck? Click **Hint** to reveal the package description. If you win or lose, the description appears automatically alongside a link to the package on CRAN.
- Use the **Play Again** button anytime to start a fresh round with a new package.

## Demo GIF

A short gameplay loop makes for a great visual introduction. If you would like to record one:

1. Run the app locally.
2. Use a screen recording tool such as OBS Studio, Peek, or Kap to capture a short round.
3. Export the clip as a GIF and add it to the `www/` directory (e.g., `www/demo.gif`).
4. Reference the GIF in this README with standard Markdown: `![HangmanneR gameplay](www/demo.gif)`.

## Bundling as an R Package

If you want to distribute HangmanneR as an installable package, the main tasks would include:

- Creating a package skeleton with `usethis::create_package()` and moving `app.R` into `inst/app/`.
- Adding the Shiny launch helper (for example, an exported `run_app()` function that calls `shiny::runApp()` on `system.file("app", package = "hangmanner")`).
- Recording dependencies in `DESCRIPTION` and listing `pkgdata.csv`, `www/`, and other assets in `inst/`.
- Providing documentation via `README.Rmd` or pkgdown, along with tests or linting as desired.

This structure keeps the interactive code untouched while making it easy for players to install the package from GitHub and run `hangmanner::run_app()` locally.
